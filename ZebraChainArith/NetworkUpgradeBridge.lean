...
Sources:
  ...
  * `ZebraChainArith/NetworkUpgrade.lean` (the cascade and indicator-sum forms)
-/
...
/-- **T4 (currentOrd at every activation height equals that upgrade's
ordinal).** Specialisation of `current_at_activation_height` to the
indicator-sum form. -/
theorem currentOrd_at_activation_height (nu : NU) :
    currentOrd (activationHeight nu) = nu.toOrd := by
  rw [← current_toOrd_eq_currentOrd (activationHeight nu),
      current_at_activation_height nu]
