...
/-!
# ZIP-1014: Dev-fund funding stream allocation (Canopy → first halving)
...
remaining `80%`. The stream runs from the first block of Canopy to the
first halving, exclusive.
...
The Rust enforcement path:
  ...
  * The active height range is `Height(1_046_400)..Height(2_726_400)` —
    Mainnet Canopy activation through (exclusive) NU6 / the first halving.
...
ZIP-1014 dev-fund arithmetic:
  ...
  3. the height range begins at Canopy (`1_046_400`) and ends *at* the
     first halving (`2_726_400`), so the stream never spans a halving,
...
/-- First block of NU6 on Mainnet, which is *also* the first halving (the
upper end of the ZIP-1014 dev-fund height range, exclusive).
Source: `zebra-chain/src/parameters/network/subsidy/constants/mainnet.rs:194`
(`…..Height(2_726_400)`). -/
...
/-- **T9 (dev-fund range starts at Canopy).** The lower bound of the
ZIP-1014 funding stream is exactly the Canopy activation height on
Mainnet (`1_046_400`).
Source: `zebra-chain/src/parameters/network/subsidy/constants/mainnet.rs:194`
+ `zebra-chain/src/parameters/network/subsidy.rs:243-246`. -/
theorem dev_fund_starts_at_canopy :
    DEV_FUND_START_HEIGHT = 1_046_400 := by
  unfold DEV_FUND_START_HEIGHT; rfl

/-- **T10 (dev-fund range ends at first halving).** The upper bound of the
ZIP-1014 funding stream is exactly `2_726_400` — the first halving height
on Mainnet, defined by the protocol as

  `slow_start_shift + 2 * PRE_BLOSSOM_HALVING_INTERVAL =
   653_600 + 2 * 840_000 = 2_333_600`?

No — for Mainnet the first halving (Canopy → halving 1) is computed at
`height_for_halving(1)` per
`zebra-chain/src/parameters/network/subsidy.rs:307-332`, which evaluates
to `2_726_400`. The dev-fund range therefore ends *exactly* at the first
halving — the stream never spans a halving boundary. -/
theorem dev_fund_ends_at_first_halving :
    DEV_FUND_END_HEIGHT = 2_726_400 := by
  unfold DEV_FUND_END_HEIGHT; rfl

/-- **T11 (dev-fund range is below the first halving).** Equivalent
restatement: every height inside the ZIP-1014 range is strictly *less* than
`DEV_FUND_END_HEIGHT`, which is exactly the first halving. So inside the
range, `halving(h) = 0` and the block subsidy is at its full pre-halving
value of `MAX_BLOCK_SUBSIDY / BLOSSOM_POW_TARGET_SPACING_RATIO` (post-
Blossom). -/
theorem dev_fund_below_first_halving (h : Nat) (hr : isInDevFundRange h) :
    h < DEV_FUND_END_HEIGHT := hr.2

/-- **T12 (dev-fund range is above Canopy).** Every height inside the
ZIP-1014 range is `≥ DEV_FUND_START_HEIGHT`, the Canopy activation height
on Mainnet.
...
/-- **T13 (range non-degenerate).** The ZIP-1014 funding stream is active
for `1_680_000` blocks — exactly the post-Blossom halving interval,
matching the Zcash whitepaper's "first halving period after Blossom" lower
bound. -/
...
/-- **T17 (first-halving height is *not* in the dev-fund range).** Sanity
check: the upper bound is exclusive, so the first halving height
(`2_726_400`) is *not* in the range. This is exactly the behaviour Zebra
encodes via Rust's `Range<Height>` (`start..end`, half-open) and is the
reason the dev-fund range "ends *at* the first halving" instead of
"continues *through* the first halving". -/
theorem first_halving_not_in_dev_fund_range :
    ¬ isInDevFundRange DEV_FUND_END_HEIGHT := by
  unfold isInDevFundRange DEV_FUND_START_HEIGHT DEV_FUND_END_HEIGHT
  decide

/-- **T18 (one-block-before-end is in the range).** The block immediately
before the first halving (`2_726_399`) *is* in the range — the very last
...
/-- **T19 (dev-fund range has the post-Blossom halving interval length).**
Restated for emphasis: the dev fund runs for exactly one full post-Blossom
halving interval (`1_680_000` blocks). -/
